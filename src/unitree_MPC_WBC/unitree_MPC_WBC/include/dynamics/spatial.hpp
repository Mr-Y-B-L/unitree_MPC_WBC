...
namespace spatial {
    ...
    */
    template<typename T>
    wbc::SXform<T> spatialRotation(CoordinateAxis axis, T theta)
    {
        static_assert(std::is_floating_point<T>::value,
                      "must use floating point value");
        wbc::RotMat<T> R = coordinateRotation(axis, theta);
        wbc::SXform<T> X = wbc::SXform<T>::Zero();
    ...
    */
    template<typename T>
    wbc::Mat6<typename T::Scalar> motionCrossMatrix(const Eigen::MatrixBase<T> &v)
    {
        static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 6,
                      "Must have 6x1 vector");
        wbc::Mat6<typename T::Scalar> m;
    ...
    */
    template<typename T>
    wbc::Mat6<typename T::Scalar> forceCrossMatrix(const Eigen::MatrixBase<T> &v)
    {
        wbc::Mat6<typename T::Scalar> f;
    ...
    */
    template<typename T>
    wbc::SVec<typename T::Scalar> motionCrossProduct(const Eigen::MatrixBase<T> &a,
                                                const Eigen::MatrixBase<T> &b)
    {
        static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 6,
                      "Must have 6x1 vector");
        wbc::SVec<typename T::Scalar> mv;
    ...
    */
    template<typename T>
    wbc::SVec<typename T::Scalar> forceCrossProduct(const Eigen::MatrixBase<T> &a,
                                               const Eigen::MatrixBase<T> &b)
    {
        static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 6,
                      "Must have 6x1 vector");
    ...
    */
    template<typename T>
    wbc::Mat4<typename T::Scalar> sxformToHomogeneous(const Eigen::MatrixBase<T> &X)
    {
        static_assert(T::ColsAtCompileTime == 6 && T::RowsAtCompileTime == 6,
                      "Must have 6x6 matrix");
        wbc::Mat4<typename T::Scalar> H = wbc::Mat4<typename T::Scalar>::Zero();
        wbc::RotMat<typename T::Scalar> R = X.template topLeftCorner<3, 3>();
        wbc::Mat3<typename T::Scalar> skewR = X.template bottomLeftCorner<3, 3>();
    ...
    */
    template<typename T>
    wbc::Mat6<typename T::Scalar> homogeneousToSXform(const Eigen::MatrixBase<T> &H)
    {
        static_assert(T::ColsAtCompileTime == 4 && T::RowsAtCompileTime == 4,
                      "Must have 4x4 matrix");
        wbc::Mat3<typename T::Scalar> R = H.template topLeftCorner<3, 3>();
        wbc::Vec3<typename T::Scalar> translate = H.template topRightCorner<3, 1>();
        wbc::Mat6<typename T::Scalar> X = wbc::Mat6<typename T::Scalar>::Zero();
        X.template topLeftCorner<3, 3>() = R;
    ...
    */
    ...
    wbc::Mat6<typename T::Scalar> createSXform(const Eigen::MatrixBase<T> &R,
    ...
    */
    template<typename T>
    wbc::RotMat<typename T::Scalar> rotationFromSXform(const Eigen::MatrixBase<T> &X)
    {
        static_assert(T::ColsAtCompileTime == 6 && T::RowsAtCompileTime == 6,
                      "Must have 6x6 matrix");
        wbc::RotMat<typename T::Scalar> R = X.template topLeftCorner<3, 3>();
    ...
    */
    template<typename T>
    wbc::Vec3<typename T::Scalar> translationFromSXform(const Eigen::MatrixBase<T> &X)
    {
        static_assert(T::ColsAtCompileTime == 6 && T::RowsAtCompileTime == 6,
                      "Must have 6x6 matrix");
        wbc::RotMat<typename T::Scalar> R = rotationFromSXform(X);
        wbc::Vec3<typename T::Scalar> r =
    ...
    */
    template<typename T>
    wbc::SXform<typename T::Scalar> invertSXform(const Eigen::MatrixBase<T> &X)
    {
        static_assert(T::ColsAtCompileTime == 6 && T::RowsAtCompileTime == 6,
                      "Must have 6x6 matrix");
        wbc::RotMat<typename T::Scalar> R = rotationFromSXform(X);
        wbc::Vec3<typename T::Scalar> r =
            -matToSkewVec(R.transpose() * X.template bottomLeftCorner<3, 3>());
        wbc::SXform<typename T::Scalar> Xinv = createSXform(R.transpose(), -R * r);
        return Xinv;
    }
    ...
    */
    template<typename T>
    wbc::SVec<T> jointMotionSubspace(JointType joint, CoordinateAxis axis)
    {
        wbc::Vec3<T> v(0, 0, 0);
        wbc::SVec<T> phi = wbc::SVec<T>::Zero();
    ...
    */
    template<typename T>
    wbc::Mat6<T> jointXform(JointType joint, CoordinateAxis axis, T q)
    {
        wbc::Mat6<T> X = wbc::Mat6<T>::Zero();
        if (joint == JointType::Revolute) {
            ...
            X = createSXform(wbc::RotMat<T>::Identity(), v);
        } else {
    ...
    */
    template<typename T>
    wbc::Mat3<typename T::Scalar> rotInertiaOfBox(typename T::Scalar mass,
                                             const Eigen::MatrixBase<T> &dims)
    {
        static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 3,
                      "Must have 3x1 vector");
        wbc::Mat3<typename T::Scalar> I =
            wbc::Mat3<typename T::Scalar>::Identity() * dims.norm() * dims.norm();
    ...
    */
    template<typename T, typename T2>
    wbc::Vec3<typename T::Scalar> spatialToLinearVelocity(const Eigen::MatrixBase<T> &v,
                                                     const Eigen::MatrixBase<T2> &x)
    {
        ...
        static_assert(T2::ColsAtCompileTime == 1 && T2::RowsAtCompileTime == 3,
                      "Must have 3x1 vector");
        wbc::Vec3<typename T::Scalar> vsAng = v.template topLeftCorner<3, 1>();
        wbc::Vec3<typename T::Scalar> vsLin = v.template bottomLeftCorner<3, 1>();
        wbc::Vec3<typename T::Scalar> vLinear = vsLin + vsAng.cross(x);
        return vLinear;
    }
    ...
    */
    template<typename T>
    wbc::Vec3<typename T::Scalar> spatialToAngularVelocity(const Eigen::MatrixBase<T> &v)
    {
        static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 6,
                      "Must have 6x1 vector");
        wbc::Vec3<typename T::Scalar> vsAng = v.template topLeftCorner<3, 1>();
    ...
    */
    template<typename T, typename T2>
    wbc::Vec3<typename T::Scalar> spatialToLinearAcceleration(const Eigen::MatrixBase<T> &a,
                                                         const Eigen::MatrixBase<T2> &v)
    {
        ...
        wbc::Vec3<typename T::Scalar> acc;
    ...
    */
                                                         ...
                                                         const Eigen::MatrixBase<T3> &x)
    {
        ...
        wbc::Vec3<typename T::Scalar> alin_x = spatialToLinearVelocity(a, x);
        wbc::Vec3<typename T::Scalar> vlin_x = spatialToLinearVelocity(v, x);

        // classical accleration = spatial linear acc + omega x v
        wbc::Vec3<typename T::Scalar> acc = alin_x + v.template head<3>().cross(vlin_x);
        return acc;
    }
    ...
    */
    template<typename T, typename T2>
    wbc::Vec3<typename T::Scalar> sXFormPoint(const Eigen::MatrixBase<T> &X,
                                         const Eigen::MatrixBase<T2> &p)
    {
        ...
        wbc::Mat3<typename T::Scalar> R = rotationFromSXform(X);
        wbc::Vec3<typename T::Scalar> r = translationFromSXform(X);
        wbc::Vec3<typename T::Scalar> Xp = R * (p - r);
        return Xp;
    }
    ...
    */
    template<typename T, typename T2>
    wbc::SVec<typename T::Scalar> forceToSpatialForce(const Eigen::MatrixBase<T> &f,
                                                 const Eigen::MatrixBase<T2> &p)
    {
        static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 3,
                      "Must have 3x1 vector");
    ...
    */
    ...
    public:
        ...
        * inertia
        */
        SpatialInertia(T mass, const wbc::Vec3<T> &com, const wbc::Mat3<T> &inertia)
        {
            wbc::Mat3<T> cSkew = vectorToSkewMat(com);
            ...
            _inertia.template bottomRightCorner<3, 3>() = mass * wbc::Mat3<T>::Identity();
        }

        /*!
        * Construct spatial inertia from 6x6 matrix
        */
        explicit SpatialInertia(const wbc::Mat6<T> &inertia) { _inertia = inertia; }

        /*!
        * If no argument is given, zero.
        */
        SpatialInertia() { _inertia = wbc::Mat6<T>::Zero(); }

        /*!
        * Construct spatial inertia from mass property vector
        */
        explicit SpatialInertia(const wbc::MassProperties<T> &a)
        {
            _inertia(0, 0) = a(4);
            _inertia(0, 1) = a(9);
            _inertia(0, 2) = a(8);
            ...
            _inertia(2, 1) = a(7);
            _inertia(2, 2) = a(6);
            wbc::Mat3<T> cSkew = vectorToSkewMat(wbc::Vec3<T>(a(1), a(2), a(3)));
            _inertia.template topRightCorner<3, 3>() = cSkew;
            _inertia.template bottomLeftCorner<3, 3>() = cSkew.transpose();
            _inertia.template bottomRightCorner<3, 3>() = a(0) * wbc::Mat3<T>::Identity();
        }
        ...
        * @param P
        */
        explicit SpatialInertia(const wbc::Mat4<T> &P)
        {
            wbc::Mat6<T> I;
            T m = P(3, 3);
            wbc::Vec3<T> h = P.template topRightCorner<3, 1>();
            wbc::Mat3<T> E = P.template topLeftCorner<3, 3>();
            wbc::Mat3<T> Ibar = E.trace() * wbc::Mat3<T>::Identity() - E;
            ...
            I.template bottomRightCorner<3, 3>() = m * wbc::Mat3<T>::Identity();
            _inertia = I;
        }
        ...
        */
        wbc::MassProperties<T> asMassPropertyVector()
        {
            wbc::MassProperties<T> a;
            wbc::Vec3<T> h = matToSkewVec(_inertia.template topRightCorner<3, 3>());
        ...
        */
        const wbc::Mat6<T> &getMatrix() const { return _inertia; }

        void setMatrix(const wbc::Mat6<T> &mat) { _inertia = mat; }

        void addMatrix(const wbc::Mat6<T> &mat) { _inertia += mat; }
        ...
        */
        ...
        {
            T m = getMass();
            wbc::Mat3<T> mcSkew = _inertia.template topRightCorner<3, 3>();
            wbc::Vec3<T> com = matToSkewVec(mcSkew) / m;
            return com;
        }
   ...
   */
        wbc::Mat3<T> getInertiaTensor()
        {
            T m = getMass();
            wbc::Mat3<T> mcSkew = _inertia.template topRightCorner<3, 3>();
            wbc::Mat3<T> I_rot = _inertia.template topLeftCorner<3, 3>() - mcSkew * mcSkew.transpose() / m;
            return I_rot;
        }
        ...
        *   Wensing, Kim, Slotine
        */
        wbc::Mat4<T> getPseudoInertia()
        {
            wbc::Vec3<T> h = matToSkewVec(_inertia.template topRightCorner<3, 3>());
            wbc::Mat3<T> Ibar = _inertia.template topLeftCorner<3, 3>();
            T m = _inertia(5, 5);
            wbc::Mat4<T> P;
            P.template topLeftCorner<3, 3>() =
                0.5 * Ibar.trace() * wbc::Mat3<T>::Identity() - Ibar;
        ...
        */
        ...
        {
            wbc::Mat4<T> P = getPseudoInertia();
            wbc::Mat4<T> X = wbc::Mat4<T>::Identity();
            if (axis == CoordinateAxis::X)
                X(0, 0) = -1;
            else if (axis == CoordinateAxis::Y)
                X(1, 1) = -1;
