...
public:
    ...
    void run(Vec34 &feetPos, Vec34 &feetVel,Vec34 &feetAcc);
    Vec3 getFootPos(int i);
    Vec3 getFootVel(int i);
    Vec3 getFootAcc(int i);
    void restart();
    //wbc
    Eigen::Matrix<double, 4, 1> getContactState();
private:
    ...
    //wbc
    float cycloidXYAcc(float startXY, float endXY, float phase);
    float cycloidZAcc(float height, float phase);
