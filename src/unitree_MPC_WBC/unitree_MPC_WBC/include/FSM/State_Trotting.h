...
public:
    ...
    bool flag_running;
    //wbc
    // Eigen::Matrix<float, 3, 1> pFoot_des[4];
    // Eigen::Matrix<float, 3, 1> vFoot_des[4];
    // Eigen::Matrix<float, 3, 1> aFoot_des[4];

    // Eigen::Matrix<float, 3, 1> pBody_des;
    // Eigen::Matrix<float, 3, 1> vBody_des;
    // Eigen::Matrix<float, 3, 1> aBody_des;

    // Eigen::Matrix<float, 3, 1> pBody_RPY_des;
    // Eigen::Matrix<float, 3, 1> vBody_Ori_des;

    // Eigen::Matrix<float, 3, 1> Fr_des[4];
    
    // Eigen::Matrix<float, 4, 1> contact_state;

    void update_wbc();
    
    qrWbcLocomotionController<float> *wbcController;
...
private:
    ...
    double _dYawCmdPast;
    Vec3 _wCmdGlobal;
    Vec34 _posFeetGlobalGoal, _velFeetGlobalGoal,_accFeetGlobalGoal;//_accFeetGlobalGoal WBC
