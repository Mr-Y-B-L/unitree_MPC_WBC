...
T FloatingBaseModel<T>::applyTestForce(const int gc_index,
                                       const wbc::Vec3<T> &force_ics_at_contact,
  ...
  dstate_out = wbc::DVec<T>::Zero(_nDof);

  // Rotation to absolute coords
  wbc::Mat3<T> Rai = _Xa[i].template block<3, 3>(0, 0).transpose();
  wbc::Mat6<T> Xc = spatial::createSXform(Rai, _gcLocation.at(gc_index));

  // D is one column of an extended force propagator matrix (See Wensing, 2012
  // ICRA)
  wbc::SVec<T> F = Xc.transpose().template rightCols<3>() * force_ics_at_contact;
...
void FloatingBaseModel<T>::udpateQddEffects() {
  ...
  for (size_t i = 6; i < _nDof; i++) {
    ...
    wbc::SVec<T> F = (_ChiUp[i].transpose() - _Xup[i].transpose()) * _S[i];
    size_t j = _parents[i];
    while (j > 5) {
...
void FloatingBaseModel<T>::updateArticulatedBodies() {
  ...
  for (size_t i = 6; i < _nDof; i++) {
    ...
    wbc::Mat6<T> XJrot = jointXform(_jointTypes[i], _jointAxes[i],
  ...
  for (size_t i = _nDof - 1; i >= 6; i--) {
    ...
    wbc::Mat6<T> Ia = _Xup[i].transpose() * _IA[i] * _Xup[i] +
...
void FloatingBaseModel<T>::addDynamicsVars(int count) {
  ...
  wbc::Mat6<T> eye6 = wbc::Mat6<T>::Identity();
  wbc::SVec<T> zero6 = wbc::SVec<T>::Zero();
  wbc::Mat6<T> zero66 = wbc::Mat6<T>::Zero();
  ...
  _J.push_back(wbc::D6Mat<T>::Zero(6, _nDof));
  _Jdqd.push_back(wbc::SVec<T>::Zero());
...
void FloatingBaseModel<T>::resizeSystemMatricies() {
  ...
  _state.q = wbc::DVec<T>::Zero(_nDof - 6);
  _state.qd = wbc::DVec<T>::Zero(_nDof - 6);
}
...
void FloatingBaseModel<T>::addBase(const SpatialInertia<T> &inertia) {
  ...
  wbc::Mat6<T> eye6 = wbc::Mat6<T>::Identity();
  wbc::Mat6<T> zero6 = wbc::Mat6<T>::Zero();
 ...
 */
template <typename T>
void FloatingBaseModel<T>::addBase(T mass, const wbc::Vec3<T> &com,
                                   const wbc::Mat3<T> &I) {
...
int FloatingBaseModel<T>::addGroundContactPoint(int bodyID,
  ...
  wbc::Vec3<T> zero3 = wbc::Vec3<T>::Zero();

  _pGC.push_back(zero3);
  _vGC.push_back(zero3);

  wbc::D3Mat<T> J(3, _nDof);
  J.setZero();
...
void FloatingBaseModel<T>::addGroundContactBoxPoints(int bodyId,
                                                     const wbc::Vec3<T> &dims) {
   addGroundContactPoint(bodyId, wbc::Vec3<T>( dims(0),  dims(1),  dims(2))/2);
   addGroundContactPoint(bodyId, wbc::Vec3<T>(-dims(0),  dims(1),  dims(2))/2);
   addGroundContactPoint(bodyId, wbc::Vec3<T>( dims(0), -dims(1),  dims(2))/2);
   addGroundContactPoint(bodyId, wbc::Vec3<T>(-dims(0), -dims(1),  dims(2))/2);

  //addGroundContactPoint(bodyId, wbc::Vec3<T>(dims(0), dims(1), 0.) / 2);
  //addGroundContactPoint(bodyId, wbc::Vec3<T>(-dims(0), dims(1), 0.) / 2);
  //addGroundContactPoint(bodyId, wbc::Vec3<T>(dims(0), -dims(1), 0.) / 2);
  //addGroundContactPoint(bodyId, wbc::Vec3<T>(-dims(0), -dims(1), 0.) / 2);
 ...
 */
template <typename T>
int FloatingBaseModel<T>::addBody(const wbc::MassProperties<T> &inertia,
                                  const wbc::MassProperties<T> &rotorInertia,
                                  T gearRatio, int parent, JointType jointType,
                                  CoordinateAxis jointAxis,
                                  const wbc::Mat6<T> &Xtree, const wbc::Mat6<T> &Xrot) {
...
void FloatingBaseModel<T>::forwardKinematics() {
  ...
  wbc::Mat3<T> R = quaternionToRotationMatrix(_state.bodyOrientation);
  _Xup[5] = spatial::createSXform(R,
                         _state.bodyPosition);
  _v[5] = _state.bodyVelocity;  // (6,)
  for (size_t i = 6; i < _nDof; i++) { // nDof = 6+12
    // joint xform
    wbc::Mat6<T> XJ = jointXform(_jointTypes[i], _jointAxes[i], _state.q[i - 6]);
    ...
    // std::cout << "i = " << i << ", _S[i]=" << "\n" << _S[i] << std::endl;
    wbc::SVec<T> vJ = _S[i] * _state.qd[i - 6];
    ...
    wbc::Mat6<T> XJrot = jointXform(_jointTypes[i], _jointAxes[i],
                               _state.q[i - 6] * _gearRatios[i]);
    _Srot[i] = _S[i] * _gearRatios[i];
    wbc::SVec<T> vJrot = _Srot[i] * _state.qd[i - 6];
  ...
  for (size_t j = 0; j < _nGroundContact; j++) {
    ...
    wbc::Mat6<T> Xai = invertSXform(_Xa[i]);  // from link to absolute
    wbc::SVec<T> vSpatial = Xai * _v[i]; // the spatial v of point in absolute frame

    // foot position in world
    _pGC.at(j) = sXFormPoint(Xai, _gcLocation.at(j));
    _vGC.at(j) = spatialToLinearVelocity(vSpatial, _pGC.at(j));
    // std::cout << "gc [" <<j<< "] " << "pGC=" << (R* (_pGC.at(j) - _state.bodyPosition)).transpose() << "  vGC=" << _vGC.at(j).transpose() << std::endl;
  }
...
// const int & link_idx, const wbc::Vec3<T> & local_pos,
// wbc::Vec3<T> & link_pos, wbc::Vec3<T> & link_vel) const {

// wbc::Mat6<T> Xai = invertSXform(_Xa[link_idx]); // from link to absolute
...
void FloatingBaseModel<T>::contactJacobians() {
  ...
  for (size_t k = 0; k < _nGroundContact; k++) {
    ...
    wbc::Mat6<T> Xc = spatial::createSXform(Rai, _gcLocation.at(k)); // Xc is binded to translated world frame WITH origin at body i COM ?

    // Bias acceleration
    wbc::SVec<T> ac = Xc * _avp[i];
    wbc::SVec<T> vc = Xc * _v[i];

    // Correct to classical
    _Jcdqd[k] = spatialToLinearAcceleration(ac, vc);

    // rows for linear velcoity in the world
    wbc::D3Mat<T> Xout = Xc.template bottomRows<3>();
 ...
 */
template <typename T>
wbc::DVec<T> FloatingBaseModel<T>::generalizedGravityForce() {
  compositeInertias();
 ...
 */
template <typename T>
wbc::DVec<T> FloatingBaseModel<T>::generalizedCoriolisForce() {
  biasAccelerations();

  // Floating base force
  wbc::Mat6<T> Ifb = _Ibody[5].getMatrix();
  wbc::SVec<T> hfb = Ifb * _v[5];
  _fvp[5] = Ifb * _avp[5] + forceCrossProduct(_v[5], hfb);

  for (size_t i = 6; i < _nDof; i++) {
    // Force on body i
    wbc::Mat6<T> Ii = _Ibody[i].getMatrix();
    wbc::SVec<T> hi = Ii * _v[i];
    _fvp[i] = Ii * _avp[i] + forceCrossProduct(_v[i], hi);

    // Force on rotor i
    wbc::Mat6<T> Ir = _Irot[i].getMatrix();
    wbc::SVec<T> hr = Ir * _vrot[i];
...
template <typename T>
wbc::Mat3<T> FloatingBaseModel<T>::getOrientation(int link_idx) {
  forwardKinematics();
  wbc::Mat3<T> Rai = _Xa[link_idx].template block<3, 3>(0, 0);
...
template <typename T>
wbc::Vec3<T> FloatingBaseModel<T>::getPosition(const int link_idx)
{
  forwardKinematics();
  wbc::Mat6<T> Xai = invertSXform(_Xa[link_idx]); // from link to absolute
  wbc::Vec3<T> link_pos = sXFormPoint(Xai, wbc::Vec3<T>::Zero());
  return link_pos;
}

template <typename T>
wbc::Vec3<T> FloatingBaseModel<T>::getPosition(const int link_idx, const wbc::Vec3<T> & local_pos)
{
  forwardKinematics();
  wbc::Mat6<T> Xai = invertSXform(_Xa[link_idx]); // from link to absolute
  wbc::Vec3<T> link_pos = sXFormPoint(Xai, local_pos);
  return link_pos;
}

template <typename T>
wbc::Vec3<T> FloatingBaseModel<T>::getLinearAcceleration(const int link_idx,
                                                    const wbc::Vec3<T> &point) {
  forwardAccelerationKinematics();
  wbc::Mat3<T> R = getOrientation(link_idx);
...
wbc::Vec3<T> FloatingBaseModel<T>::getLinearAcceleration(const int link_idx) {
  forwardAccelerationKinematics();
  wbc::Mat3<T> R = getOrientation(link_idx);
  return R * spatialToLinearAcceleration(_a[link_idx], _v[link_idx], wbc::Vec3<T>::Zero());
}


template <typename T>
wbc::Vec3<T> FloatingBaseModel<T>::getLinearVelocity(const int link_idx,
                                                const wbc::Vec3<T> &point) {
  forwardKinematics();
  wbc::Mat3<T> Rai = getOrientation(link_idx);
...
wbc::Vec3<T> FloatingBaseModel<T>::getLinearVelocity(const int link_idx) {
  forwardKinematics();
  wbc::Mat3<T> Rai = getOrientation(link_idx);
  return Rai * spatialToLinearVelocity(_v[link_idx], wbc::Vec3<T>::Zero());
}



template <typename T>
wbc::Vec3<T> FloatingBaseModel<T>::getAngularVelocity(const int link_idx) {
  forwardKinematics();
  wbc::Mat3<T> Rai = getOrientation(link_idx);
  // wbc::Vec3<T> v3 =
...
template <typename T>
wbc::Vec3<T> FloatingBaseModel<T>::getAngularAcceleration(const int link_idx) {
  forwardAccelerationKinematics();
  wbc::Mat3<T> Rai = getOrientation(link_idx);
 ...
 */
template <typename T>
wbc::DMat<T> FloatingBaseModel<T>::massMatrix() {
  compositeInertias();
  _H.setZero();
  ...
  for (size_t j = 6; j < _nDof; j++) { // _nDof = 18
    ...
    wbc::SVec<T> f = _IC[j].getMatrix() * _S[j];
    wbc::SVec<T> frot = _Irot[j].getMatrix() * _Srot[j];
...
void FloatingBaseModel<T>::forwardAccelerationKinematics() {
  ...
  wbc::SVec<T> aGravity = wbc::SVec<T>::Zero();
 ...
 */
template <typename T>
wbc::DVec<T> FloatingBaseModel<T>::inverseDynamics(
    const FBModelStateDerivative<T> &dState) {
  setDState(dState);
  forwardAccelerationKinematics();

  // Spatial force for floating base
  wbc::SVec<T> hb = _Ibody[5].getMatrix() * _v[5];
  _f[5] = _Ibody[5].getMatrix() * _a[5] + forceCrossProduct(_v[5], hb);

  // loop through joints
  for (size_t i = 6; i < _nDof; i++) {
    // spatial momentum
    wbc::SVec<T> hi = _Ibody[i].getMatrix() * _v[i];
    wbc::SVec<T> hr = _Irot[i].getMatrix() * _vrot[i];
...
template <typename T>
void FloatingBaseModel<T>::runABA(const wbc::DVec<T> &tau,
  ...
  wbc::SVec<T> ivProduct = _Ibody[5].getMatrix() * _v[5];
  ...
  for (size_t i = 5; i < _nDof; i++) {
    ...
    wbc::Mat3<T> R = rotationFromSXform(_Xa[i]);
    wbc::Vec3<T> p = translationFromSXform(_Xa[i]);
    wbc::Mat6<T> iX = spatial::createSXform(R.transpose(), -R * p);
  ...
  wbc::SVec<T> a0 = -aGravity;
  wbc::SVec<T> ub = -_pA[5];
  _a[5] = _Xup[5] * a0;
  wbc::SVec<T> afb = _invIA5.solve(ub - _IA[5].transpose() * _a[5]);
  _a[5] += afb;

  // joint accelerations
  dstate.qdd = wbc::DVec<T>(_nDof - 6);
  for (size_t i = 6; i < _nDof; i++) {
    dstate.qdd[i - 6] =
  ...
  // output
  wbc::RotMat<T> Rup = rotationFromSXform(_Xup[5]);
 ...
 */
template <typename T>
wbc::DMat<T> FloatingBaseModel<T>::invContactInertia(
    const int gc_index, const wbc::D6Mat<T> &force_directions) {
  forwardKinematics();
  updateArticulatedBodies();
  updateForcePropagators();
  ...
  // ICRA)
  wbc::D6Mat<T> D = Xc.transpose() * force_directions;

  size_t m = force_directions.cols();

  wbc::DMat<T> LambdaInv = wbc::DMat<T>::Zero(m, m);
  wbc::DVec<T> tmp = wbc::DVec<T>::Zero(m);
